        ...
        void DoLoadProcessesSnapshot()
        {
            ...
            try
            {
                ...
                if (Process32First(handleToSnapshot, ref procEntry))
                {
                    do
                    {
                        System.Diagnostics.Debug.Write(procEntry.th32ProcessID + " Name " + procEntry.szExeFile + "\n");
                    } while (Process32Next(handleToSnapshot, ref procEntry));
                }
                else
                {
                    throw new ApplicationException(string.Format("Failed with win32 error code {0}",
